...
def get_nightscout_authorization_token():
    ...
    try:
        r = requests.get(
            nightscout_host + "/api/v2/authorization/request/" + token_secret
        )
...
def check_permissions():
    ...
    if pg == ["*"]:  # admin role
        logging.warning(
            "The use of the admin role for token based authentication is not recommended, see https://openaps.readthedocs.io/en/master/docs/walkthrough/phase-1/nightscout-setup.md#switching-from-api_secret-to-token-based-authentication-for-your-rig"  # noqa E501
        )
    else:
        missing = []
        for perm in [
        ...
        if len(missing) > 0:
            logging.error(
                "The following permissions are missing in Nightscout: %s" % missing
            )
            logging.error(
                "Please follow instructions at https://openaps.readthedocs.io/en/master/docs/walkthrough/phase-1/nightscout-setup.md#switching-from-api_secret-to-token-based-authentication-for-your-rig"  # noqa E501
            )
            sys.exit(1)
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description="Checks permissions in Nightscout based on your ns.ini"
        )
        parser.add_argument(
            "-v", "--verbose", action="store_true", help="increase output verbosity"
        )
        parser.add_argument(
            "--nsini", type=str, help="Path to ns.ini", default="./ns.ini"
        )
