...
def get_input_arguments():
    ...
    parser.add_argument(
        "--dir", "-d", type=str, required=True, help="(--dir=<OpenAPS Directory>)"
    )
    parser.add_argument(
        "--ns-host",
        "-n",
...
def get_nightscout_profile(nightscout_host, directory):
    ...
    with open(
        os.path.join(directory, "autotune", "nightscout.profile.json"), "w"
    ) as f:  # noqa: F821
...
def get_openaps_profile(directory):
    shutil.copy(
        os.path.join(directory, "settings", "pumpprofile.json"),
    ...
    # If a previous valid settings/autotune.json exists, use that; otherwise start from settings/profile.json
    # cp settings/autotune.json autotune/profile.json && cat autotune/profile.json | json | grep -q start ||
    # cp autotune/profile.pump.json autotune/profile.json
    # create_autotune_json = "cp {0}settings/autotune.json {0}autotune/profile.json &&
    # cat {0}autotune/profile.json | json | grep -q start ||
    # cp {0}autotune/profile.pump.json {0}autotune/profile.json".format(directory)
    ...
    shutil.copy(
        os.path.join(directory, "settings", "autotune.json"),
        os.path.join(directory, "autotune", "pumpprofile.json"),
    )
...
def get_nightscout_carb_and_insulin_treatments(
    nightscout_host, start_date, end_date, directory
):
    logging.info(
        "Grabbing NIGHTSCOUT treatments.json for date range: {0} to {1}".format(
            start_date, end_date
        )
    )
    ...
    url = '{0}/api/v1/treatments.json?find\\[created_at\\]\\[\\$gte\\]=`date --date="{1} -4 hours" -Iminutes`&find\\[created_at\\]\\[\\$lte\]=`date --date="{2} +1 days" -Iminutes`'.format(  # noqa E501
...
def get_nightscout_bg_entries(nightscout_host, start_date, end_date, directory):
    logging.info(
        "Grabbing NIGHTSCOUT enries/sgv.json for date range: {0} to {1}".format(
    ...
    for run_number in range(1, number_of_runs + 1):
        for date in date_list:
            ...
            shutil.copy(
                ...
                os.path.join(
                    autotune_directory,
                    "profile.{run_number}.{date}.json".format(
                        run_number=run_number, date=date.strftime("%Y-%m-%d")
                    ),
                ),
            )
            ...
            # oref0-autotune-prep ns-treatments.json profile.json ns-entries.$DATE.json >
            ...
            ns_entries = os.path.join(
                autotune_directory,
                "ns-entries.{date}.json".format(date=date.strftime("%Y-%m-%d")),
            )

            # autotune.$RUN_NUMBER.$DATE.json
            autotune_run_filename = os.path.join(
                autotune_directory,
                "autotune.{run_number}.{date}.json".format(
                    run_number=run_number, date=date.strftime("%Y-%m-%d")
                ),
            )
            autotune_prep = "oref0-autotune-prep {ns_treatments} {profile} {ns_entries} {pump_profile} --output-file {autotune_run_filename}".format(  # noqa E501
                ns_treatments=ns_treatments,
                profile=profile,
                ns_entries=ns_entries,
                pump_profile=pump_profile,
                autotune_run_filename=autotune_run_filename,
            )
            logging.info("Running {script}".format(script=autotune_prep))
            call(autotune_prep, stdout=FNULL, shell=True)
            logging.info(
                "Writing output to {filename}".format(filename=autotune_run_filename)
            )
            ...
            # oref0-autotune-core autotune.$RUN_NUMBER.$DATE.json profile.json profile.pump.json >
            # newprofile.$RUN_NUMBER.$DATE.json

            # oref0-autotune-core autotune.$run_number.$i.json profile.json profile.pump.json >
            # newprofile.$RUN_NUMBER.$DATE.json
            profile_pump = os.path.join(autotune_directory, "profile.pump.json")

            # newprofile.$RUN_NUMBER.$DATE.json
            newprofile_run_filename = os.path.join(
                autotune_directory,
                "newprofile.{run_number}.{date}.json".format(
                    run_number=run_number, date=date.strftime("%Y-%m-%d")
                ),
            )
            autotune_core = "oref0-autotune-core {autotune_run} {profile} {profile_pump} --output-file {newprofile_run_filename}".format(  # noqa E501
                profile=profile,
                profile_pump=profile_pump,
                autotune_run=autotune_run_filename,
                newprofile_run_filename=newprofile_run_filename,
            )
            logging.info("Running {script}".format(script=autotune_core))
            call(autotune_core, stdout=FNULL, shell=True)
            logging.info(
                "Writing output to {filename}".format(filename=newprofile_run_filename)
            )
...
def export_to_excel(output_directory, output_excel_filename):
    autotune_export_to_xlsx = (
        "oref0-autotune-export-to-xlsx --dir {0} --output {1}".format(
            output_directory, output_excel_filename
        )
    )
...
def create_summary_report_and_display_results(output_directory):
    ...
    report_file = os.path.join(
        output_directory, "autotune", "autotune_recommendations.log"
    )
    autotune_recommends_report = "oref0-autotune-recommends-report {0}".format(
        output_directory
    )
...
if __name__ == "__main__":
    ...
    get_nightscout_carb_and_insulin_treatments(
        nightscout_host, start_date, end_date, directory
    )
