...
def parseDateAndRun(filename):
    m = re.match(
        r"profile.(?P<run>.*).(?P<date>20[0-9][0-9]-[01][0-9]-[0-3][0-9]).json",
        filename,
    )
    if m:
...
def expandProfile(l, valueField, offsetField):  # noqa E741 - TODO: fix ambiguous variable name
    r = []
    minutes = 0
    ...
    for i in range(len(l)):
        ...
        if minutes1 != offset1:
            print(
                "Error in JSON offSetField %s contains %s does not match start time %s (%d minutes). "
                "Please report this as a bug"
                % (offsetField, offset1, start1, minutes1)
            )
            sys.exit(1)
...
def excel_init_workbook(workbook):
    ...
    excel_hour_format = workbook.add_format(
        {"num_format": "hh:mm", "bold": True, "font_color": "black"}
    )
    excel_2decimals_format = workbook.add_format(
        {"num_format": "0.00", "font_size": "16"}
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Export oref0 autotune files to Microsoft Excel"
    )
    parser.add_argument("-d", "--dir", help="autotune directory", default=".")
    parser.add_argument(
        "-o", "--output", help="default autotune.xlsx", default="autotune.xlsx"
    )
    ...
    (
        worksheetBasal,
        worksheetIsf,
        excel_2decimals_format,
        excel_integer_format,
    ) = excel_init_workbook(workbook)
    row = 1  # start on second row, row=0 is for headers
    filenamelist = glob.glob("profile.json")
    filenamelist.extend(glob.glob("profile.pump.json"))
    filenamelist.extend(glob.glob("profile.[0-9].*.json"))
    filenamelist.extend(glob.glob("profile.[0-9][0-9].*.json"))
