...
def index():
    try:
        myopenaps_dir = os.environ["OPENAPS_DIR"]  # noqa F841
    except KeyError:
        myopenaps_dir = "/root/myopenaps/"   # noqa F841
    data = dict()
    try:
        error_text = "getHost"
        ...
        if os.path.isfile("/tmp/pump_loop_success"):
            data["loop_completed"] = datetime.fromtimestamp(
                os.path.getmtime("/tmp/pump_loop_success"), pytz.utc
            )
        else:
            data["loop_completed"] = ""
...
def glucose():
    if (
        os.path.getmtime(myopenaps_dir + "xdrip/glucose.json")
        > os.path.getmtime(myopenaps_dir + "monitor/glucose.json")
        and os.path.getsize(myopenaps_dir + "xdrip/glucose.json") > 0
    ):
...
def sgvjson():
    ...
    if os.path.getmtime(myopenaps_dir + "xdrip/glucose.json") > os.path.getmtime(
        myopenaps_dir + "monitor/glucose.json"
    ):
        json_url = os.path.join(myopenaps_dir + "xdrip/glucose.json")
    else:
        json_url = os.path.join(myopenaps_dir + "monitor/glucose.json")
